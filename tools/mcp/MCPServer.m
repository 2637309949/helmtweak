...
// HelmMCP port startup rule:
//   - probe port before bind
//   - free        -> proceed with bind
//   - ours-leftover (responds to MCP tools/list with our tool signature) -> post stop, wait, re-probe
//   - foreign     -> abort, no +1 fallback
typedef NS_ENUM(NSInteger, HelmMCPPortState) {
    HelmMCPPortStateFree = 0,
    HelmMCPPortStateOurs,
    HelmMCPPortStateForeign,
};

static HelmMCPPortState HelmMCPProbePort(uint16_t port) {
    int probe = socket(AF_INET, SOCK_STREAM, 0);
    if (probe < 0) return HelmMCPPortStateFree;
    ...
    close(probe);
    if (rc != 0) {
        // ECONNREFUSED: nothing listening -> free. Timeout/other: treat as free, let bind fail naturally.
        (void)savedErrno;
        return HelmMCPPortStateFree;
    }

    // Port occupied. Probe whether it speaks our MCP protocol by sending tools/list
    // and looking for the press_volume_up tool signature unique to this dylib.
    ...
    NSURLSessionDataTask *task = [session dataTaskWithRequest:req completionHandler:^(NSData *data, NSURLResponse *resp, NSError *err) {
        ...
        if (data && [resp isKindOfClass:[NSHTTPURLResponse class]] && [(NSHTTPURLResponse *)resp statusCode] == 200) {
            ...
            if ([json isKindOfClass:[NSDictionary class]] && [json[@"jsonrpc"] isEqualToString:@"2.0"]) {
                NSArray *tools = json[@"result"][@"tools"];
                if ([tools isKindOfClass:[NSArray class]]) {
                    for (NSDictionary *t in tools) {
                        if ([t isKindOfClass:[NSDictionary class]] && [t[@"name"] isEqualToString:@"press_volume_up"]) {
                            isOurs = YES;
                            break;
                        }
                    }
    ...
    [task resume];
    dispatch_semaphore_wait(sem, dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.7 * NSEC_PER_SEC)));
...
- (void)startOnPort:(uint16_t)port {
    ...
    if (state == HelmMCPPortStateOurs) {
        MCP_LOG(@"Port %d occupied by our own leftover; posting stop and retrying", port);
        CFNotificationCenterPostNotification(CFNotificationCenterGetDarwinNotifyCenter(),
                                            IOS_MCP_DARWIN_NOTIFICATION_STOP, NULL, NULL, true);
        for (int i = 0; i < 15; i++) {
            usleep(100000); // 100ms
            if (HelmMCPProbePort(port) == HelmMCPPortStateFree) break;
        }
        state = HelmMCPProbePort(port);
    }
    if (state == HelmMCPPortStateForeign) {
        MCP_LOG(@"Port %d occupied by foreign (non-MCP) process; aborting start (no +1 fallback)", port);
        return;
    }
    if (state == HelmMCPPortStateOurs) {
        MCP_LOG(@"Port %d still occupied by our leftover after stop; aborting start", port);
        return;
    }
...
- (NSDictionary *)handleInitialize:(id)reqId params:(NSDictionary *)params {
    ...
    return @{
        ...
        @"result": @{
            ...
            @"instructions": MCPUsageGuideString(),
        }
    };
}

static NSString *MCPUsageGuideString(void) {
    // 使用指引放在独立资源文件，便于编辑而不重编译。
    // rootless 路径经 HelmSystemInfo 解析（/var/jb/usr/share/helmtweak/usage_guide.txt）。
    NSString *path = [HelmSystemInfo rootfs:@"/usr/share/helmtweak/usage_guide.txt"];
    if (path.length > 0) {
        NSError *error = nil;
    ...
    return @"Usage guide unavailable (missing /usr/share/helmtweak/usage_guide.txt). "
           @"Call tools/list for the tool inventory; each tool description explains its usage.";
}
...
- (NSDictionary *)handleToolsCall:(id)reqId params:(NSDictionary *)params {
    ...
    } else if ([toolName isEqualToString:@"get_usage_guide"]) {
        return [self mcpSuccess:reqId text:MCPUsageGuideString()];
    }
