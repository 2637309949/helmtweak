...
#define OCR_LOG(fmt, ...) [HelmLogger log:@"[OCR] " fmt, ##__VA_ARGS__]

@implementation HelmOCRManager
...
+ (instancetype)sharedInstance {
    static HelmOCRManager *instance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[HelmOCRManager alloc] init];
    });
    return instance;
}
...
- (NSDictionary *)recognizeTextWithLanguages:(NSArray<NSString *> *)languages
    ...
    if (@available(iOS 13.0, *)) {
        UIImage *image = [[HelmScreenManager sharedInstance] captureScreenImage];
