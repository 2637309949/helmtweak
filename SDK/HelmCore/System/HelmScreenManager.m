...
#define SCREEN_LOG(fmt, ...) do { \
    if ([HelmLogger isDebugLoggingEnabled]) { \
        NSString *_helm_log = [NSString stringWithFormat:(@"[Screen] " fmt), ##__VA_ARGS__]; \
...
@interface HelmScreenManager ()
- (NSDictionary *)deviceInteractionStateOnMainThread;
@end

@implementation HelmScreenManager
...
+ (instancetype)sharedInstance {
    static HelmScreenManager *instance;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        instance = [[HelmScreenManager alloc] init];
    });
    return instance;
}
...
- (NSDictionary *)privateScreenshotPayload {
    ...
    NSData *screenData = [HelmScreenManager getScreenDataWithQuantity:(NSInteger)round(kMCPScreenshotInitialJPEGQuality * 100.0)];
